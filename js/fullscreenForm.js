...
;( function( window ) {
	...
	FForm.prototype._init = function() {
        ...
        // the section element
        this.sectionEl = this.el.querySelector( 'section');
		...
		var self = this,
			onEndAnimationFn = function( ev ) {
				...
				if( self.isLastStep ) {
                    ...
                    classie.add( self.fieldsList, 'fs-remove' );
                    classie.add( self.sectionEl, 'fs-show' );
	...
	FForm.prototype._progress = function() {
		if( this.options.ctrlProgress ) {
            ...
            if( this.current === 2 ) {
                ...
                /*if( this.current === 2 ) {
                    document.getElementById('full-bar').style.marginRight = Math.abs(this.current * 300) * -1 + 'px';
                    document.getElementById('full-bar').style.width = this.current * 300 + 'px';
                    document.getElementById('first').className += ' done';
                    function UpdateCircle() {
                        document.getElementById('second').className += ' heart';
                    }
                    document.getElementById('first').className = document.getElementById('first').className.replace(/\bheart\b/g, "");
                    setTimeout(UpdateCircle, 300);
